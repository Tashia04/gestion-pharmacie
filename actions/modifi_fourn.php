...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    if (empty($id) || empty($nom) || empty($adresse) || empty($telephone) || empty($email) || empty($produits_fournis)) {
        ...
        try {
            ...
            $executed = updateFournisseur($id, $nom, $adresse, $telephone, $email, $produits_fournis);

            if ($executed) {
                echo "<script>alert('Le fournisseur a été mis à jour avec succès.');window.location.href='../pages/affiche_fourn.php';</script>";
            } else {
